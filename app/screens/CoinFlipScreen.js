...
function CoinFlipScreen(props) {
  ...
  const eventHandler = () => {
    ...
    if (numFlip == 1) {
      ...
      //finish and now set the hexagram
  ...
  const showComponent = (props) => {
    ...
    if (numFlip > 6) {
      ...
      return (
        <Button
          title="Read your hexagram"
          color="#008080"
          onPress={() =>
            props.navigation.navigate("Analysis", { hexObj, lineObj })
          }
        />
      ); //pass in the results
    }
  ...
  const showFlipCoin = (props) => {
    //after 6 flips turn off the button
    if (numFlip <= 6) {
      return (
  ...
  return (
    ...
    >
      <SafeAreaView style={styles.container}>
        ...
        <View>{showFlipCoin(props)}</View>

        <View style={styles.analysisButton}>{showComponent(props)}</View>
      </SafeAreaView>
    </ImageBackground>
  );
}
