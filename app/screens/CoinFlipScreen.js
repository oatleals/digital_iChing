...
import { Image, ImageBackground, SafeAreaView, Button, View } from 'react-native'
...
function CoinFlipScreen(props) {
  ...
  const showComponent = (props) => { //send data to Analysis
    if (numFlip > 6) {
      ...
      return <Button title="Read your hexagram" color="#008b8b" onPress=
  ...
  const showFlipCoin = (props) => { //after 6 flips turn off the button
    if (numFlip <= 6) {
      return <Button title="Flip coins" color="#008b8b" onPress={eventHandler} />
    }
  }
